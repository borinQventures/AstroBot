...
function collectSymlinkPaths(rootPath) {
    const symlinks = [];
    // ⚡ Bolt: Use withFileTypes to avoid lstatSync per file during directory traversal
    function walkDir(dirPath, relativeDir) {
        const entries = (0, node_fs_1.readdirSync)(dirPath, { withFileTypes: true });
        for (const entry of entries) {
            const nextRelative = relativeDir ? node_path_1.default.join(relativeDir, entry.name) : entry.name;
            if (entry.isSymbolicLink()) {
                symlinks.push(nextRelative);
            }
            else if (entry.isDirectory()) {
                const nextPath = node_path_1.default.join(dirPath, entry.name);
                walkDir(nextPath, nextRelative);
            }
        }
    }
    const rootStat = (0, node_fs_1.lstatSync)(rootPath);
