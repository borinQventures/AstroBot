...
// ⚡ Bolt: [performance improvement] Optimized directory traversal by replacing `lstatSync` inside
// the loop with `readdirSync({ withFileTypes: true })`, reducing file system calls from O(N) to O(1)
// for stat lookups when gathering symlinks.
